...
async def parse_job_description(
    ...
    try:
        if text.strip() == "":
            raise HTTPException(status_code=400, detail="Please insert the Job Description text.")
...
@router.post("/interview-question-generation/", response_model=InterviewQuestionResponse)
...
@router.post("/question-evaluation-generation/", response_model=InterviewQuestionResponse)
async def qa_evaluation_and_gen(
    ...
    try:
        ...
        print(f"Request: {request}")
        def generate():
            for chunk in qa_evaluation_and_generation(
                request=request
            ):
                yield chunk
