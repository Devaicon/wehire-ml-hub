...
def get_uploaded_file_names(directory: str | Path, extension: str | None = None) -> List[str]:
    """
...
def load_json_from_file(directory: str | Path, filename: str) -> dict:
    ...
    file_path = upload_dir / (filename if filename.endswith(".json") else f"{filename}.json")
